...
public abstract class BossObject<T> : MonoBehaviour, IDamagableBoss, IPoolable where T : BossObject<T>
{
    ...
    protected virtual void OnTriggerEnter2D(Collider2D _other)
    {
        ...
        if (!_other.gameObject.TryGetComponent(out IShootable shootable)) return;
        DisablePoolable();
